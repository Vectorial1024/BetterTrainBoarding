...
namespace BetterTrainBoarding
{
    ...
    {
        private static void AnalyzeTrain(ushort vehicleID, out bool isFull, out List<CompartmentInfo> analysis)
        {
        ...
        public static bool PreFix(ushort vehicleID, ref Vehicle data, ushort currentStop, ushort nextStop)
        {
            ...
            bool isFull;
            List<CompartmentInfo> analysis;
            AnalyzeTrain(vehicleID, out isFull, out analysis);
            if (!isFull)
            {
                // this gets triggered 99% of the time, but eh who cares.
                SensiblyLoadPassengers(vehicleID, currentStop, nextStop, analysis);
                return false;
            }
            return true;
        }
